...
class SocialSecurityNumber extends Component {
  ...
  componentWillMount() {
    ...
    if (get(workflow, 'state.data') === undefined) {
      ...
      this.setState({
        prevssn: get(workflow, 'state.data.ssn') ? conformToMask(`${get(workflow, 'state.data.ssn')}`, [/\d/, /\d/, /\d/, '-', /\d/, /\d/, '-', /\d/, /\d/, /\d/, /\d/]).conformedValue : null,
      });
    }

    if (!params.applicationId) {
      history.push('/applications/dtm/application');
    }
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (!isEqual(nextProps.workflow, this.props.workflow)) {
      this.setState({
        prevssn: get(nextProps.workflow, 'state.data.ssn') ? conformToMask(`${get(nextProps.workflow, 'state.data.ssn')}`, [/\d/, /\d/, /\d/, '-', /\d/, /\d/, '-', /\d/, /\d/, /\d/, /\d/]).conformedValue : null,
      });
    }
  }
  ...
  render() {
    const { isLoading, prevssn } = this.state;
              ...
              <Row>
                <Col sm={{ size: 8, offset: 2 }} md={{ size: 10, offset: 1 }} lg={{ size: 8, offset: 2 }}>
                  <Card className="dark-shadow">
                    <CardBody>
                      <h4 className="mb-3">Please Re-enter Your Social Security Number</h4>
                      {
                        prevssn &&
                          <p>You entered {prevssn}</p>
                      }
                      <Form>
